...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/07 16:53:52 by awyart            #+#    #+#             */
/*   Updated: 2017/06/07 17:04:13 by awyart           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_iter(t_env *env)
{
	...
	while (++i < env->iter)
	{
		...
		tmpy = 2 * env->yn * env->xn;
		env->xn = tmpx * env->xn - tmpy * env->yn
			+ 0.1 * (double)env->sizex / ((double)env->option_x);
		env->yn = tmp * tmpy + env->yn * tmpx
